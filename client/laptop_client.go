...
//LaptopClient is  struct to call laptop service RPC's
type LaptopClient struct {
	service pb.LaptopServiceClient
}

//NewLaptopClient is the ctor
...
func (laptopClient *LaptopClient) CreateLaptop(laptop *pb.Laptop) {
	...
	//set timeout for invalid requests
...
//SearchLaptop searches for a laptop based on config
func (laptopClient *LaptopClient) SearchLaptop(filter *pb.Filter) {
	...
	if err != nil {
		log.Fatal("cannot reach laptop: ", err)
	}
...
//RateLaptop gives rating to laptop
func (laptopClient *LaptopClient) RateLaptop(laptopIDs []string, scores []float64) error {
	...
	//go routine to recieve responses in parllel
	go func() {
		for {
			res, err := stream.Recv()
			if err == io.EOF {
			...
			log.Print("recueved response: ", res)
		}
	}()

	//send requests in parllel
...
func (laptopClient *LaptopClient) UploadImage(laptopID string, path string) {
	...
	if err != nil {
		log.Fatal("cannot uplaod image: ", err)
	}
	...
	for {
		...
		err = stream.Send(req)
		if err != nil {
			log.Fatal("cannot read chunk to server:", err, stream.RecvMsg(nil))
		}
		...
		log.Printf("image uploaded with id: %s, size: %d", response.GetId(), response.GetSize())
	}
}
